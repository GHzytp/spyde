...
- `external/quantem/` is the **vendored matcher** (`orientation.py`, `crystal.py`, `rotations.py`, …) — byte-identical to upstream apart from import rewrites, pinned by `_sync.py` to a commit on the fork that carries the unreleased `diffraction` subpackage. **Do not edit these files**; `_sync.py` fails if an unhandled `quantem.*` import appears. Adaptation belongs on the SpyDE side.
- `vector_orientation_quantem.py`: **the production path**, and everything SpyDE-specific about it — `phase_to_crystal` (orix `Phase` → ase → `Crystal`; NOT `Crystal.from_cif`, ase rejects `F m 3 m`), `PeaksAdapter` (duck-types quantem's ragged `Vector` over our CSR buffer, converting to Å⁻¹ once), `strain_from_orientation_map` (batched; real space is the inverse transpose), and `SinglePatternFitter` — one pattern in ~55 ms, which is what makes the live preview possible. `install_zone_angle_cache()` memoises the symmetry reduction (184.5 → 44.6 ms per single fit).
- `vector_orientation_om.py` is the wizard; the whole-field fit runs on **Run**, not Generate — the full map is too slow to recompute live, while a single pattern under the navigator is not. `vector_refine_ipf.py` is the IPF correlation heat map (26 ms, so it runs inline on the nav dispatcher).
- The per-pattern scipy pose fit and its batched-Adam GPU twin that preceded this are **deleted**. They reported strain in the OPPOSITE sense, so do not reintroduce one as a fallback — `test_vector_orientation_om.py` asserts both modules fail to import.
...
**Batch the whole problem, don't loop.** The vectors and template library for a 4D-STEM scan are only a few MB. The win is transferring everything to the GPU once and running *every* nav position in lockstep as one batched tensor op — not dask, not processes, not a per-pattern Python loop. The orientation matcher is the model: pack all P patterns → `(P, …)` tensors, correlate every zone axis at once, and take all in-plane angles as one FFT axis rather than a loop over angles.
...
- **Long GPU work hands the device back at a bounded interval**, it does not hold the lock end-to-end. Each path picks the finest granularity it can: the EBSD refine wraps the caller's `on_yield` so every yield does `mps_sync()` → release → yield → re-acquire (a concurrent preview waits ~12 refine steps); the spectrum fit releases between chunks; `compute_vector_orientation_quantem` takes the lock per STAGE (plan build, match, refine+strain) because the batched stages have no interior yield point. **Always `mps_sync()` before releasing** — handing off while kernels are still in flight lets the next thread submit into a live encoder, which is the race itself.
...
- Existing harnesses live in `spyde/tests/benchmark_*.py` (run directly with `python -m spyde.tests.benchmark_<name>`, not under pytest — they're slow). `benchmark_quantem_orientation.py` finds vectors on the calibrated sped_ag scan and times each stage of the match — the reference for the vector OM path.
...
- `torch`-CUDA work **segfaults under the pytest process on Windows** (a harness interaction, not a code bug — it runs fine in plain Python and in the real app). So: run GPU correctness tests in a **subprocess** that prints a JSON result (see `test_neural_detect.py`), and `os._exit(0)` after printing to skip the torch/CUDA teardown crash. GUI tests that exercise the *wiring* should force the CPU path (`monkeypatch gpu_available → False`).
