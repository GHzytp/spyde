...
aim and the deduplication that makes the dispatcher wait for a decode already
running rather than start a second one.
"""
...
from spyde.drawing.update_functions import (
    _block_prefetcher, _next_chunk_position, _step_signs,
)
...
class TestPrefetcher:
    def test_it_decodes_the_chunk_the_drag_is_heading_into(self):
        data = da.zeros((256, 256, 4, 4), chunks=(8, 8, 4, 4),
                        dtype=np.float32)
        signal = _Signal(data, 2)
        reader = _ChunkedReader(data)
